...
namespace delay_plugin {
    ...
    struct get_param_type<float> {
        using value = juce::AudioParameterFloat;
    };
    template<>
    struct get_param_type<bool> {
        using value = juce::AudioParameterBool;
    };
    ...
    struct ParameterList {
        ParameterList(Vts::ParameterLayout &layout)
            : _layout(layout) {};
    ...
    struct SmoothParam {
        ...
        SmoothParam(juce::ParameterID id)
            : id(id) {}

        void getFromTree(Vts &vt) {
            param =
                    dynamic_cast<ParamaType *>(vt.getParameter(id.getParamID()));
        }
    };
    ...
    public:
        DelayParameters(Vts &state) {
            ...
            _tempoSyncParam = dynamic_cast<juce::AudioParameterBool *>(
                    state.getParameter(tempoSyncParamID.getParamID()));
            _delayNoteParam = dynamic_cast<juce::AudioParameterChoice *>(
                    state.getParameter(delayNoteParamID.getParamID()));
            _delayParam = dynamic_cast<juce::AudioParameterFloat *>(
                    state.getParameter(delayParamID.getParamID()));
        }
            ...
            layout.add(
                    std::make_unique<juce::AudioParameterFloat>(
                            ...
                            juce::AudioParameterFloatAttributes()
                                    .withStringFromValueFunction(stringFromDb)),
                    std::make_unique<juce::AudioParameterFloat>(
                            delayParamID, "Delay",
                            juce::NormalisableRange<float>{MIN_DELAY_TIME.count(),
                                                           MAX_DELAY_TIME.count(),
                                                           0.001f, 0.25f},
                            100.0f,
                            juce::AudioParameterFloatAttributes()
                                    .withStringFromValueFunction(stringFromTime)),
                    std::make_unique<juce::AudioParameterFloat>(
                            mixParamID, "mix",
                    ...
                    std::make_unique<juce::AudioParameterFloat>(
                            ...
                            juce::AudioParameterFloatAttributes()
                                    .withStringFromValueFunction(stringFromPercent)),
                    std::make_unique<juce::AudioParameterFloat>(
                            lowCutParamID, "lowCut",
                            juce::NormalisableRange<float>{20.0f, 20000.0f, 1.0f, 0.3f},
                            20.0f,
                            juce::AudioParameterFloatAttributes()
                                    .withStringFromValueFunction(stringFromHz)),
                    std::make_unique<juce::AudioParameterFloat>(
                            highCutParamID, "highCut",
                            juce::NormalisableRange<float>{20.0f, 20000.0f, 1.0f, 0.3f},
                            20000.0f,
                            juce::AudioParameterFloatAttributes()
                                    .withStringFromValueFunction(stringFromHz)),
                    std::make_unique<juce::AudioParameterChoice>(
                            delayNoteParamID, "Delay Note", noteLengths,
                            noteLengths.size()),
        ...
        void update() noexcept {
            _gain.smooth.setTargetValue(
                    juce::Decibels::decibelsToGain(_gain.param->get()));
        ...
        void reset() noexcept {
            ...
            *_highCut.param = 2000.0f;
            ...
            _gain.smooth.setCurrentAndTargetValue(
                    juce::Decibels::decibelsToGain(_gain.param->get()));
        ...
        float gain() noexcept { return _gain.smooth.getNextValue(); }

        float delay() noexcept { return _delay; }

        float mix() noexcept { return _mix.smooth.getNextValue(); }

        float feeback() { return _feedback.smooth.getNextValue(); }
        std::tuple<float, float> pan() {
            _pan = getPanningEqualPower(_stereo.smooth.getNextValue());
            return _pan;
        }

        float lowCut() { return _lowCut.smooth.getNextValue(); }

        float highCut() { return _highCut.smooth.getNextValue(); }

        float tempoSync() { return _tempoSync; }

        int delayNote() { return _delayNote; }
